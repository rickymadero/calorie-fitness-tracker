  ...
  return (
    <aside className="hidden w-60 shrink-0 flex-col border-r border-border bg-nav text-nav-fg lg:flex">
      ...
      <nav className="flex flex-1 flex-col gap-1 overflow-y-auto p-3">
        ...
        <Link
          href="/posts/new"
          className="mt-2 flex min-h-11 items-center justify-center gap-2 rounded-xl bg-accent px-3 py-2.5 text-sm font-semibold text-accent-fg"
        >
          <Plus size={18} />
          Share workout
        </Link>
        ...
        {MOBILE_TABS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex min-h-[52px] flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-medium ${
